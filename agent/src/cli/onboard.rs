...
- 如果任务引用本地输入文件，先检查文件是否存在且可访问；文件不存在时直接说明，不要先自举环境
- 如果任务需要 Python，先确认输入文件存在，再决定是否在工作区内自举、写脚本、执行、返回结果
...
fn generate_example_skills(skills_dir: &Path) -> anyhow::Result<()> {
    ...
    for (name, content) in skills {
        ...
        if !path.exists() {
            ...
            println!("  → skills/{name}/SKILL.md ✓");
        } else {
            println!("  → skills/{name}/SKILL.md (已存在，跳过)");
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_generate_example_skills_creates_directory_packages() {
        let dir = std::env::temp_dir().join("anqclaw_test_onboard_example_skills");
        let _ = std::fs::remove_dir_all(&dir);

        generate_example_skills(&dir).unwrap();

        assert!(dir.join("code-review").join("SKILL.md").is_file());
        assert!(dir.join("git-workflow").join("SKILL.md").is_file());
        assert!(dir.join("summarize").join("SKILL.md").is_file());
        assert!(!dir.join("code-review.md").exists());
        assert!(!dir.join("git-workflow.md").exists());
        assert!(!dir.join("summarize.md").exists());
